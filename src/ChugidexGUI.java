...
/**
 * This class implements a graphic GUI application for the cs300 Fall 2022 p09 
 Chugidex program
 *
...
public class ChugidexGUI extends PApplet {
  ...
  /**
   * Defines the initial environment properties of this graphic application, loads
   * media such as
  ...
  public void setup() {
    ...
    if (SOUND_ON) {
      SoundFile opening = new SoundFile(this, "sounds" + File.separator +
              "opening.mp3");
      opening.loop();
  ...
  private int getChugimonKey(Chugimon chugi) {
    ...
    return ((chugi.getFIRST_ID() + chugi.getSECOND_ID())
            * (chugi.getFIRST_ID() + chugi.getSECOND_ID() + 1) / 2) +
            chugi.getSECOND_ID();
  }
  ...
  private PImage getChugimonImage(Chugimon chugi) {
    ...
    PImage chugiImg =
            this.loadImage("./images/" + chugi.getFIRST_ID() + "." + chugi.getSECOND_ID()
                    + ".png");
  ...
  private void drawChugimon(Chugimon chugi) {
    ...
    this.image(chugiImage, width / 2 - SCREEN_WIDTH / 2, height / 2 - SCREEN_HEIGHT
                    / 2,
  ...
  private void drawChugimonDescription(Chugimon chugi) {
    ...
    String idText = "#" + chugi.getFIRST_ID() + "." + chugi.getSECOND_ID() + "";
    if (chugi.getFIRST_ID() == chugi.getSECOND_ID()) {
      nameText = "MissingNo.";
      idText = "#0";
    }
  ...
  private void drawScreen() {
    this.noStroke();
    this.fill(WHITE);
    this.rect(width / 2 - BEZEL_WIDTH / 2, height / 2 - BEZEL_HEIGHT / 2,
            BEZEL_WIDTH, BEZEL_HEIGHT,
            SCREEN_CORNER_RADIUS);
    this.fill(BLACK);
    this.rect(width / 2 - SCREEN_WIDTH / 2, height / 2 - SCREEN_HEIGHT / 2,
            SCREEN_WIDTH,
  ...
  private void drawDPad() {
    this.noStroke();
    this.fill(BLACK);
    this.rect(width - DPAD_OFFSET + DPAD_MINOR_AXIS, height - DPAD_OFFSET,
            DPAD_MINOR_AXIS,
            DPAD_MAJOR_AXIS, DPAD_CORNER_RADIUS);
    this.rect(width - DPAD_OFFSET, height - DPAD_OFFSET + DPAD_MINOR_AXIS,
            DPAD_MAJOR_AXIS,
  ...
  private void drawMinisreen() {
    this.noStroke();
    this.fill(GREEN);
    this.rect(MINISCREEN_CORNER_X, MINISCREEN_CORNER_Y, MINISCREEN_WIDTH,
            MINISCREEN_HEIGHT,
